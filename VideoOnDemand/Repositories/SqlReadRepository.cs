...
namespace VideoOnDemand.Repositories
{
    public class SqlReadRepository : IReadRepository
    {
        ...
        public Course GetCourse(string userId, int courseId)
        {
            ...
            course.Modules = _db.Modules.Where(m => m.CourseId.Equals(courseId)).ToList();

            foreach (var module in course.Modules)
            {
                module.Downloads = _db.Downloads.Where(d => d.ModuleId.Equals(module.Id)).ToList();
                module.Videos = _db.Videos.Where(d => d.ModuleId.Equals(module.Id)).ToList();
            }
        ...
        {
            var video = _db.Videos.Where(v => v.Id.Equals(videoId))
            .Join(_db.UserCourses, v => v.CourseId, uc => uc.CourseId,
